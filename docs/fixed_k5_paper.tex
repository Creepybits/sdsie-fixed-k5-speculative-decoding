...
against a recurrence. In a supplementary benchmark (Section~\ref{sec:vllm-comparison}),
we additionally compare this same mechanism against vLLM's own native
\texttt{draft\_model} speculative decoding under matched conditions, finding
a +22\% to +100\% throughput improvement over vLLM's own FP16 baseline when
run natively inside vLLM's serving engine.
\end{abstract}
...
-- a different mechanism in kind from the fixed-$K$ approach reported here,
closer in spirit to the entropy-gated dynamic mechanism excluded from this
paper (see Introduction) than to fixed-$K$5 itself; it is not benchmarked
in this paper. vLLM, by contrast, ships a \texttt{draft\_model}
speculative-decoding method that is mechanistically the same approach as
this paper's own (small draft model, fixed draft window, target-model
verification) -- Section~\ref{sec:vllm-comparison} reports a direct
head-to-head against it. Our own testing of an entropy-gated variant of
\emph{our} mechanism, using the same hardware and methodology as this
paper, has not yet demonstrated an improvement over the simpler fixed-$K$
approach reported here; we regard that as an open question requiring
further work (e.g. testing under multi-request/batched serving, which
published entropy-gating results suggest is where the technique's value
proposition may actually lie), not a settled negative result. This paper
makes no claims about that entropy-gated mechanism in either direction.
...
common \texttt{bench\_common.py} module, and reused by the supplementary
vLLM benchmark in Section~\ref{sec:vllm-comparison}.
...
production-representative. Section~\ref{sec:vllm-comparison} reports a
separate, KV-cached comparison as a distinct, non-substitutable result.
...
\subsection{Comparison to vLLM's Native Speculative Decoding}
\label{sec:vllm-comparison}

vLLM ships its own native speculative decoding, including a
\texttt{draft\_model} method with a fixed \texttt{num\_speculative\_tokens}
setting -- mechanistically the same approach as this paper's own (small
draft model, fixed draft window, target model verifies). As a supplementary
benchmark, we compare this paper's fixed-$K{=}5$ mechanism directly against
it: the same scout (Llama-3.2-1B-Instruct) and target
(Llama-3.1-8B-Instruct) models, the same three reference prompts, vLLM's
own engine and KV-cache for both arms, $N{=}10$ trials per prompt, greedy
decoding. Table~\ref{tab:vllm} reports the result.

\begin{table}[t]
\centering
\caption{Fixed-$K{=}5$ mechanism vs. vLLM's own native speculative decoding, run natively inside vLLM (both arms KV-cached). N=10 trials/prompt.}
\label{tab:vllm}
\begin{tabular}{lccc}
\toprule
Prompt & vLLM base. tok/s & vLLM+K5 tok/s & Speedup \\
\midrule
Poem     & 100.6 & 123.1 & +22.4\% \\
Physics  & 100.8 & 151.8 & +50.6\% \\
Code     & 100.8 & 201.4 & \textbf{+99.9\%} \\
\bottomrule
\end{tabular}
\end{table}

The fixed-$K{=}5$ mechanism measurably outperforms vLLM's own FP16 baseline
when run natively inside vLLM's serving engine, across all three reference
prompts. Three caveats apply. First, this result is \textbf{not directly
comparable} to Table~\ref{tab:results}: vLLM uses PagedAttention
(KV-caching) on both arms here, whereas Table~\ref{tab:results}
deliberately omits caching on both arms for the different reason described
in Section~\ref{sec:warmup-correction} (isolating the speculative mechanism
from caching effects) -- these are two distinct, both methodologically
valid comparisons answering different questions, and their baseline numbers
should not be read as points on the same axis. Second, this benchmark
covers vLLM only, not SGLang; SGLang's own built-in speculative decoding is
adaptive (EAGLE-based, tiered draft length), a different mechanism closer
in kind to the entropy-gated dynamic approach this paper explicitly
excludes than to fixed-$K$5, and has not been benchmarked here. Third, this
is a single full run, not yet independently cross-validated by a second
script the way Table~\ref{tab:results} is corroborated by
\texttt{speculative\_scout.py}; the same closed-loop, per-prompt warmup
convergence procedure described in Section~\ref{sec:warmup-correction} was
reused for this benchmark (via the shared \texttt{bench\_common.py}
module), and a first-round rotation-order artifact found during development
of this benchmark (whichever prompt ran first in a trial round measured
slower, independent of which specific prompt it was) was diagnosed and
fixed using that same per-label convergence check before the result above
was collected.
...
As noted in Methodology, neither arm of the main comparison
(Table~\ref{tab:results}) uses KV-caching. A production serving stack with
proper cache reuse would show substantially higher absolute throughput in
both arms. Section~\ref{sec:vllm-comparison}'s vLLM comparison does use a
production-representative KV-cached serving path, but is a distinct
benchmark against a different baseline (vLLM's own), not a KV-cached
extension of Table~\ref{tab:results}; a KV-cached variant of this paper's
own manual ablation harness, directly comparable to Table~\ref{tab:results},
remains future work and is the primary remaining gap before these results
can inform a deployment decision in full.
...
under an Apache-2.0 license. Reported numbers in Table~\ref{tab:results}
are directly traceable to \texttt{telemetry/ablation\_results.json},
generated by \texttt{benchmark\_ablation.py} in that repository; Table~\ref{tab:vllm}
is directly traceable to \texttt{telemetry/vllm\_comparison\_results.json},
generated by \texttt{benchmark\_vllm\_comparison.py}. The closed-loop warmup
and drift-diagnostic logic described in Sections~\ref{sec:warmup-correction}
and~\ref{sec:validity} is shared between all three scripts via
\texttt{bench\_common.py}.
...
zero loss of output fidelity. A supplementary benchmark further shows this
same mechanism outperforming vLLM's own native speculative decoding by
+22\% to +100\% when run inside vLLM's serving engine
(Section~\ref{sec:vllm-comparison}), though that comparison remains a
single run pending independent cross-validation and does not cover SGLang.
We report this as a validated, narrow result, distinct from and not
dependent on the more experimental entropy-gated dynamic mechanism under
investigation elsewhere in the parent project.
