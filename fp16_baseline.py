...
Plain FP16 target-only decoding, used as the comparison baseline for
speculative_scout.py and benchmark_ablation.py. Same warmup procedure and
power-sampling methodology as the other scripts in this repo, so comparisons
are apples-to-apples: same NVML monitor class, same warmup steps, same
prompts, same trial-aggregation approach.
"""
...
def run_n_trial_suite(num_trials=5, max_target_tokens=250, warmup_steps=5):
    ...
    summary_json = os.path.join(TELEMETRY_DIR, f"fp16_baseline_n{num_trials}_summary.json")
    with open(summary_json, "w") as f:
        json.dump(aggregated, f, indent=2)

    print("\n" + "=" * 95)
    print(f"N={num_trials} FP16 BASELINE COMPLETE -- MEAN +/- STD")
